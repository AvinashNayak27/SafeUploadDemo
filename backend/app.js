...
app.post('/uploadWithBundlr', async (req, res) => {
  const data = req.body;
  try {
    const contentURI = await uploadWithBundlr(data);
    res.send({ message: 'Data uploaded successfully', contentURI });
    console.log('Data uploaded successfully');
  } catch (error) {
    res.status(500).send({ message: 'Error uploading data', error: error.message });
...
async function uploadWithBundlr(data) {
